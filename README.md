...
`app/analytics/candidate_evidence.py` materializes one daily row per stable candidate identity: `trading_day + symbol + direction + setup`. It consolidates repeated scan observations with suggestion status, paper-trade status, replay outcome, target/stop-first flags, winner/missed-winner labels, option quality, trend health, trend capture, Trade Efficiency Score (TES), quote freshness, and engineering root cause. `decision` preserves the first actionable `ENTER` or `ENTER_PAPER` decision seen that day; `latest_decision` shows the newest observed state; `decision_history` records each decision transition with its timestamp, scan ID, reason, and blocker. Evidence merges durable `candidate_snapshot` rows, local daily snapshots, and the current scan with scan-level deduplication, so a later `WAIT` cannot erase an earlier actionable recommendation. Every finalized scan writes the local `candidate_evidence.csv` and `candidate_evidence_status.json`; the status records expected and written rows, repeated source observations collapsed into stable candidate identities, and database rows persisted. Postgres promotion is optional and its explicit status is recorded as `DISABLED`, `PERSISTED`, or `FAILED` rather than inferred from the local artifact.
...
The daily activity trace is the chronological operational record for every evaluated ticker. It keeps five separate layers: **Scanner Recommendation** (`ENTRY_RECOMMENDED`), **Execution Eligibility/Outcome/Reason**, **Trade Status** (`NOT_CREATED` or `OPEN`), and **Telegram Status/Reason** (`NO_LIFECYCLE_EVENT`, `SENT`, or `NOT_SENT`). Legacy `Action Status=ENTER_PAPER` remains available for compatibility, but no longer implies a completed paper entry. The trace also combines one event per evaluated rule with stage/actual/required/pass-fail values, paper lifecycle events, and Telegram delivery events using a stable event ID. Each scanner decision retains the prior action, whether the state changed, action reason/gate, setup score, RR, option quality, and the exact completed 5-minute decision candle (time, open, high, low, close, volume). The local `activity_trace.csv` remains the review artifact, and each finalized scan queues an idempotent best-effort upsert to Postgres table `activity_trace_event`; migrations `016_activity_trace.sql`, `017_activity_trace_decision_context.sql`, `019_execution_eligibility.sql`, and `020_trade_notification_layers.sql` provide indexed lookup by trading day/symbol/time, trade/time, scan, state transition, and each layer's state. Database unavailability never blocks scanner completion, paper state, or Telegram delivery.

Each `ENTRY_RECOMMENDED` scanner fact is also stored append-only in `data/recommendation_outcomes/recommendation_facts.csv` and Postgres `recommendation_fact`. Later scans evaluate the same underlying after 5 and 10 business sessions, writing `recommendation_horizon_outcomes.csv` and Postgres `recommendation_horizon_outcome`. The summary groups recommendations into rank buckets `1-3`, `4-10`, and `11+` and reports execution rate, directional win rate, average directional return, and option-return coverage. Option return is populated only when the exact original option ticker has a refreshed mid-price; otherwise it is explicitly marked `OPTION_CONTRACT_NOT_REFRESHED`.

Multiday positions use a conservative profit-protection layer in the V1 exit engine: after a `+2R` peak it locks `+1R`; after a `+3R` peak, a `1R` giveback exits the position. The values are configurable through `MULTIDAY_PROFIT_LOCK_MFE_R`, `MULTIDAY_PROFIT_LOCK_R`, `MULTIDAY_PROFIT_EXIT_MFE_R`, and `MULTIDAY_PROFIT_MAX_GIVEBACK_R`. Delayed stock data remains an automated `STALE_STOCK_DATA` rejection when it exceeds the configured freshness limit; it never expects an operator to validate charts or quotes. Daily Review Exports include `high_score_execution_audit.csv`, listing high-score recommendations with rank, execution outcome, and exact blocker.
...
Validation baseline: `python -m unittest discover -t . -s tests` currently runs 174 tests. **Use `-t . -s tests`, not `discover tests`**: only the package-importing form runs `tests/__init__.py`, which redirects `DRAVYA_DATA_DIR` / `DRAVYA_STATE_DIR` to a sandbox. The bare `discover tests` form imports test modules top-level, skipping that bootstrap, and the run then writes into the real `data/` and `app/state/` artifacts. `pytest tests` is isolated either way via `tests/conftest.py`. The intentional background-worker failure printed by `test_background_queue.py` verifies failure isolation and does not fail the suite.
...
- The focused fallback validation is `d:/Dravya_Trade_Works/.venv/Scripts/python.exe -m unittest tests.test_option_liquidity_fallback`. Full local validation uses `d:/Dravya_Trade_Works/.venv/Scripts/python.exe -m unittest discover -t . -s tests`.
