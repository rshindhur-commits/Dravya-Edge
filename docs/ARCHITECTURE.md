...
| 6 | Validation baseline "134 tests" | **140 tests**, all passing | `python -m unittest discover tests` |
...
| 15 | "Test coverage is still sparse" vs "134 tests pass" | **Both are true, not a contradiction.** 140 tests pass, and 93 of ~160 `app/` modules have no direct test import — high count, uneven coverage. Substantiated in [`_inventory.md` §4.1](_inventory.md#41-modules-with-no-direct-test-import) | `unittest discover`, AST import map |
