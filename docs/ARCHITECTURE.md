...
| 6 | Validation baseline "134 tests" | **367 tests**, all passing under pytest. The legacy `unittest discover` command collects only 310 — it cannot see the files using bare `def test_*` functions | `python -m pytest tests -q` |
...
| 15 | "Test coverage is still sparse" vs "134 tests pass" | **Both are true, not a contradiction.** 367 tests pass, and 93 of ~160 `app/` modules have no direct test import — high count, uneven coverage. Substantiated in [`_inventory.md` §4.1](_inventory.md#41-modules-with-no-direct-test-import) | `pytest`, AST import map |
| 16 | "No CI exists" | **Partially superseded by S2.5.** `.github/workflows/ci.yml` now runs the suite and an I1-enforcement gate on every push/PR. It does not cover deployment, linting, or type-checking — "no CI" was accurate through Phase 2 S2.4; treat any remaining "fails CI" plan language for those other concerns as still needing a pipeline | [`docs/specs/S2.5-strategy-version-gate.md`](specs/S2.5-strategy-version-gate.md) |
