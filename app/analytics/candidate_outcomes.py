...
def build_candidate_outcomes(trading_day):
    ...
    direction = _first_existing(audit, ["direction", "Direction", "Candidate Direction"])
    ...
    for _, row in audit.iterrows():
        ...
        candidate_id = hashlib.sha256(
            f"{trading_day}|{row.get(symbol)}|{row.get(direction) if direction else ''}|{row.get(setup) if setup else ''}".encode()
        ).hexdigest()[:24]
    ...
    return outcomes.drop_duplicates("candidate_id", keep="last").reset_index(drop=True)
