...
class CandidateSnapshotRepository(BestEffortRepository):
    def batch_insert(self, snapshots, trading_day=None, scan_id=None):
        ...
        return self._batch_execute("""
            INSERT INTO candidate_snapshot (trading_day, scan_id, symbol, direction, setup, score, regime, entry_readiness, risk_reward, option_quality, candidate_rank, action, blocked_reason, realtime_ready, execution_ready, candidate_persistence, created_at)
            VALUES (:trading_day, :scan_id, :symbol, :direction, :setup, :score, :regime, :entry_readiness, :risk_reward, :option_quality, :candidate_rank, :action, :blocked_reason, :realtime_ready, :execution_ready, :candidate_persistence, now())
        """, [{
            "trading_day": row.get("trading_day"), "scan_id": row.get("scan_id"), "symbol": row.get("symbol"), "direction": row.get("direction"), "setup": row.get("setup_type"), "score": row.get("setup_percent"), "regime": row.get("market_regime"), "entry_readiness": raw.get("ENTRY_READINESS"), "risk_reward": row.get("candidate_rr"), "option_quality": row.get("option_quality_score"), "candidate_rank": raw.get("Candidate Rank"), "action": row.get("action_status"), "blocked_reason": row.get("blocked_by"), "realtime_ready": raw.get("Realtime Ready"), "execution_ready": raw.get("Execution Ready"), "candidate_persistence": raw.get("Candidate Persistence"),
