...
class RuleEvaluationRepository(BestEffortRepository):
    def batch_insert(self, evaluations):
        return self._batch_execute("""
            INSERT INTO rule_evaluation (scan_id, symbol, setup, rule_name, rule_group, actual_value, required_value, passed, blocked_trade, priority, timestamp)
            VALUES (:scan_id, :symbol, :setup, :rule_name, :rule_group, CAST(:actual_value AS TEXT), CAST(:required_value AS TEXT), :passed, :blocked_trade, :priority, now())
