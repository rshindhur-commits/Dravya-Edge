...
- `persist_scan_artifacts_db` promotes candidate snapshots, structured rule evaluations, existing gate-decision summaries, scanner-run finish state, and missed-winner attribution as best-effort database batches. `app/gates/rule_evaluation.py` creates queryable `RuleEvaluation` records with rule name/group, actual/required values, pass/fail, blocked state, and priority from entry-gate diagnostics.
...
- Promoted immutable tables are `candidate_snapshot`, `rule_evaluation`, `entry_snapshot`, `exit_snapshot`, `trade_timeline_event`, and `candidate_outcome`. Existing `alert_events`, `paper_trades`, `scanner_runs`, and `gate_decisions` remain for compatibility. Failed DB writes do not block Telegram, scanner output, paper-trade JSON/CSV state, validation reports, or Streamlit rendering.
- Entry snapshots are created once after the paper-entry event; exit snapshots are created once after exit snapshot/trend-capture artifacts. `trade_timeline.jsonl` mirrors append-only entry/exit events. Candidate outcomes are generated post-validation from audit/replay data, making Telegram misses and false alerts objective derived facts.
- `app/db/migrations/001_promote_scanner_artifacts.sql` must be applied manually through `DATABASE_DIRECT_URL` before enabling the promoted tables. The scanner never runs DDL automatically.
