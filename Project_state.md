...
- `telegram_dispatch` records dispatcher ATTEMPT, SENT, and FAILED transitions with scan/trade/symbol metadata, message type, decision, attempt/delivery status, failure reason, and Telegram message ID when available. JSONL audit remains at `data/live/telegram_dispatch_audit.jsonl`; each DB row is queued through `RuntimeScheduler` and cannot delay delivery.
