...
- `derive_holding_profile()` runs before the shared decision adapter returns a `TradeDecision`. It prefers an explicit candidate value and otherwise derives a multi-day profile from expected-hold intent or an eligible 14-30+ DTE, high-quality setup; all other candidates default to `INTRADAY`.
...
- `Auto Exit` owns only normal stop, target, live-exit, invalidation, and profit-threshold checks. EOD automation is a separate, final holding-policy consumer: with `Auto Close Intraday Trades` enabled, the scheduler closes only the trade's current `INTRADAY` profile and leaves `MULTIDAY` positions open. A manual profile change is therefore honored at EOD without mutating the frozen entry thesis.
- `Restore Multi-day Positions` defaults to on. When disabled, it skips multi-day restoration and continuation-alert setup but still archives stale candidates at session start.
...
- The Trading page displays separate Intraday and Multi-day open-trade counts. Multi-day positions send a single next-session `POSITION CONTINUES` Telegram alert rather than a second `NEW TRADE` alert.
...
- Candidate snapshots and Candidate Evidence persist Entry Timing score/grade/reason, TQS, Entry Priority Adjustment, Expected Remaining Trend, Projected Entry Grade, Ranking Score, and rank so outcome research can compare these values with winners, Trend Capture %, and TES. The Trading Decision Center exposes TQS and Entry Timing, Validation exposes timing distributions, late entries, rankings, exit waterfalls, and a Decision Waterfall, and Reports aggregates timing/TQS/rank from daily Validation caches.
...
- Telegram exit alerts are allowed for explicitly tracked `PAPER`, `REAL`, and confirmed `SCANNER_TRACKED` V1 lifecycle trades. Successful exit alerts set `exit_alert_sent` and use deterministic duplicate keys built from event type, symbol, option ticker, opened time, and exit reason.
