...
- Telegram entry alerts consume the scanner action as their only business eligibility check. `ENTER`, `ENTER_PAPER`, and `REVIEW_TV_CHART` are alertable when entry alerts are enabled.
...
- `app/analytics/trade_ranker.py` computes an observational `Trade Quality Score` (TQS): Setup 25%, Entry Timing 20%, Trend Health 20%, Option Quality 15%, Relative Strength 10%, and Liquidity 10%. `Candidate Rank` and `Rank Reason` are attached after relative-strength enrichment without reordering scanner execution or alerts.
- Candidate snapshots and Candidate Evidence persist Entry Timing score/grade/reason, TQS, and rank so outcome research can compare these values with winners, Trend Capture %, and TES. The Trading Decision Center exposes TQS and Entry Timing, Validation exposes timing distributions, late entries, rankings, exit waterfalls, and a Decision Waterfall, and Reports aggregates timing/TQS/rank from daily Validation caches.
...
- All of these fields are observational. They cannot alter V1 entry/exit execution, Telegram eligibility, or paper/real trade state. Continue to require at least 20 evidence days and 80 completed trades before controlled rule-change review.
- V2 exit shadow additionally records `exit_confidence_score`, a five-state health label (`VERY_HEALTHY`, `HEALTHY`, `WEAKENING`, `AT_RISK`, `FAILED`), soft confirmation set/count/streak, and Grace Zone status. Hard stop/target stay immediate; soft deterioration may enter `MONITOR` until confirmations persist. This never changes V1 exits.
...
- Telegram sends an entry alert whenever the scanner action is `ENTER`, `ENTER_PAPER`, or `REVIEW_TV_CHART` and entry alerts are enabled. It does not use `TELEGRAM_ALERT_POLICY`, conviction, score, option, quote, event, regime, session, or top-candidate checks as entry-alert eligibility. It also sends dashboard paper-entry opens, full exit alerts for scanner-managed and paper-trade closes, and one-time partial-profit alerts when scanner trade management reaches the partial threshold.
