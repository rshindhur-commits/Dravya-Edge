...
- Auto-paper decision logs distinguish scanner recommendation from execution outcome: `action_status` records the scanner action, `blocked_by` records the actual auto-paper gate that prevented an open, `scanner_blocked_by` preserves the raw scanner field, and `action_reason` preserves the scanner explanation. An action code such as `ENTER_PAPER` is never used as the execution blocker for a new blocked record.
