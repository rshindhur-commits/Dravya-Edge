...
- After `df_results` and the health payload are built, `run_scanner()` queues `_persist_scan_outputs()` through `RuntimeScheduler` as a high-priority `RuntimeJob`. That job writes dashboard state, engine health history, DB gate decisions, candidate snapshots, signal lifecycle rows, scanner output files, scanner-run finish state, and scanner stage profiles.
