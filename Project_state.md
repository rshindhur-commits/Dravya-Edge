...
- Streamlit Secrets DB keys should be root-level values. The dashboard also maps `[connections.trading_db].url` into `DATABASE_URL` as a fallback, but `DB_WRITE_ENABLED` and pool settings still need root-level secrets.
